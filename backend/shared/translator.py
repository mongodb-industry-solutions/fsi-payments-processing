...
CANONICAL COPY — edit this file first, then sync to the per-service copies:
  - backend/payment_agent/services/translator.py
  - backend/payment_converter_v2/src/services/translator.py
The per-service copies exist because each service has its own Docker context
that does not include backend/shared/.
