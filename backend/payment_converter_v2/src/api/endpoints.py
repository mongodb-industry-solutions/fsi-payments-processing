...
class DetailedConversionResponse(BaseModel):
    """Response model for detailed conversion endpoint with processing breakdown"""
    output: str = Field(..., description="Converted message")
    processing_stats: ProcessingStats = Field(..., description="Processing lane statistics")
    confidence_scores: Dict[str, float] = Field(..., description="AI confidence scores")
    human_review_required: bool = Field(..., description="Whether human review is needed")
    metadata: Dict[str, Any] = Field(..., description="Conversion metadata")
    detailed_processing: Dict[str, Any] = Field(..., description="Detailed processing information (extraction, RULES lane, AI lane)")
...
@router.post("/convert-detailed", response_model=DetailedConversionResponse, status_code=status.HTTP_200_OK)
async def convert_message_detailed(request: ConversionRequest) -> DetailedConversionResponse:
    """
    Convert a payment message with detailed processing information.

    This endpoint provides the same conversion as /convert but includes:
    - Field extraction details (patterns used, values extracted)
    - RULES lane transformation details (transform type, input/output values)
    - AI lane processing details (prompts, responses, confidence breakdown)

    Useful for understanding and debugging the conversion process.
    """
    try:
        logger.info(f"Converting message with details: {request.conversion_id}")
        ...
        # Perform conversion with detailed information
        result = await converter.convert(
            source_format=source_format,
            target_format=target_format,
            message=request.message,
            include_details=True  # Request detailed processing information
        )

        # Extract lane distribution
        stats = result["processing_stats"]
        lane_dist = stats.get("lane_distribution", {})
        ...
        metadata = result["metadata"].copy()
        metadata["conversion_id"] = request.conversion_id
        metadata["conversion_type"] = "direct"  # Single-hop conversion
        metadata["conversion_path"] = f"{source_format} → {target_format}"
        metadata["processing_time"] = metadata.pop("processing_time_seconds")

        # Map to detailed response model
        return DetailedConversionResponse(
            output=result["converted_message"],
            processing_stats=ProcessingStats(
                rules_lane=lane_dist.get("RULES", 0),
                ai_lane=lane_dist.get("AI", 0),
            ...
            human_review_required=result["human_review_required"],
            metadata=metadata,
            detailed_processing=result.get("detailed_processing", {})
        )
        ...
        logger.error(f"Detailed conversion failed: {str(e)}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Conversion failed: {str(e)}"
        )
...
@router.post("/convert/multi-hop-detailed", response_model=DetailedConversionResponse, status_code=status.HTTP_200_OK)
async def convert_multi_hop_detailed(request: MultiHopConversionRequest) -> DetailedConversionResponse:
    """
    Multi-hop conversion with detailed processing information for both hops.

    Example: MT103 → JSON → pacs.008

    Returns detailed breakdown for:
    - Hop 1 (Source → JSON): extraction, rules_lane, ai_lane, configuration
    - Hop 2 (JSON → Target): extraction, rules_lane, ai_lane, configuration
    - Multi-hop metadata: path, timing, caching status

    This enables complete visibility into the multi-hop conversion process
    for visualization in the frontend.
    """
    try:
        # Generate unique ID for this conversion run
        conversion_run_id = str(uuid.uuid4())
        logger.info(f"Multi-hop detailed: {request.source_format} → JSON → {request.target_format}")

        # Track agent corrections
        agent_correction = None

        # Hop 1: Source → JSON with detailed processing
        try:
            hop1_result = await converter.convert(
                source_format=request.source_format,
                target_format="JSON",
                message=request.message,
                conversion_run_id=conversion_run_id,
                include_details=True  # Enable detailed processing
            )

            logger.info("Hop 1 complete with detailed processing")
            ...
            # Build agent_result from final state
            if not final_state.get("result"):
                raise HTTPException(
                    status_code=500,
                    detail=f"Payment agent failed to correct violation: {e.reason}"
            ...
            # Build hop1_result from cached corrected JSON
            hop1_result = {
                'conversion_id': f"{request.source_format}_to_JSON",
                'request_id': None,
                'converted_message': cached_json['json_data'],
                ...
                'detailed_processing': {}  # No details available for cached corrected JSON
            }

            logger.info(f"Using corrected JSON from MongoDB cache")

        # Hop 2: JSON → Target with detailed processing
        hop2_result = await converter.convert(
            source_format="JSON",
            target_format=request.target_format,
            message=hop1_result['converted_message'],
        ...
        logger.info("Hop 2 complete with detailed processing")

        # Build multi-hop details
        hop1_time = hop1_result['metadata']['processing_time_seconds']
        hop2_time = hop2_result['metadata']['processing_time_seconds']
            ...
            "hops": [
                {
                    ...
                    "to": "JSON",
                    "config_id": f"{request.source_format}_to_JSON",
                    "processing_time": hop1_time,
                    "cached": False
                },
                {
                    "step": 2,
                    ...
                    "config_id": f"JSON_to_{request.target_format}",
                    "processing_time": hop2_time,
                    "cached": True  # JSON was read from cache
                }
            ],
            "conversion_run_id": conversion_run_id
        }
        ...
        # Combine metadata
        combined_metadata = hop2_result['metadata'].copy()
        combined_metadata['conversion_type'] = 'multi_hop'
        combined_metadata['conversion_path'] = f"{request.source_format} → JSON → {request.target_format}"
        combined_metadata['multi_hop'] = True
        combined_metadata['multi_hop_details'] = multi_hop_details
        combined_metadata['hop1_time'] = hop1_time
        combined_metadata['hop2_time'] = hop2_time
        combined_metadata['total_time'] = hop1_time + hop2_time
        combined_metadata['agent_correction'] = agent_correction

        # Combine processing stats from both hops
        hop1_stats = hop1_result.get("processing_stats", {})
        hop2_stats = hop2_result.get("processing_stats", {})

        hop1_lane_dist = hop1_stats.get("lane_distribution", {})
        hop2_lane_dist = hop2_stats.get("lane_distribution", {})

        combined_stats = ProcessingStats(
            rules_lane=hop1_lane_dist.get("RULES", 0) + hop2_lane_dist.get("RULES", 0),
            ai_lane=hop1_lane_dist.get("AI", 0) + hop2_lane_dist.get("AI", 0),
            human_lane=hop1_lane_dist.get("HUMAN", 0) + hop2_lane_dist.get("HUMAN", 0)
        )

        # Build detailed processing with both hops
        detailed_processing = {
            "hop1_details": hop1_result.get("detailed_processing", {}),
            "hop2_details": hop2_result.get("detailed_processing", {}),
            # For convenience, also include hop2 at top level for backward compatibility
            **hop2_result.get("detailed_processing", {})
        }
        ...
        logger.error(f"Multi-hop detailed conversion failed: {str(e)}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Multi-hop detailed conversion failed: {str(e)}"
        )
