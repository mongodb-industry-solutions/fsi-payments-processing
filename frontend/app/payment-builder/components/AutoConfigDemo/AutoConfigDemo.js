...
-}`,
        mt205_to_pacs009: `{1:F01CHASUS33AXXX0000000000}{2:I205DEUTDEFFXXXXN}{3:{108:COV}}{4:
:20:MT205TEST001
:21:COVREF2024001
:13C:/SNDTIME/1415+0100
...
/INS/COV PAYMENT FOR CLIENT
/REC/NOTIFY TREASURY DEPT
...
-}`
  ...
  const handleScenarioSelect = (selected) => {
    ...
    if (selected.isCustom) {
      ...
      const samples = {
        mt192_to_pacs008: `{1:F01CHASUS33XXXX0000000000}...`, // abbreviated for brevity
        mt205_to_pacs009: `{1:F01CHASUS33AXXX0000000000}...`,
        mt202cov_to_pacs009: `{1:F01BOFAUS3NXXXX0000000000}...`
      };
  ...
  const handleStartAutoConfig = async () => {
    ...
    try {
      ...
      const config = (isCustom || currentScenario?.isCustom) ? customConfig : {
        sourceFormat: currentScenario.sourceFormat || currentScenario.name?.split(' → ')[0],
        targetFormat: currentScenario.targetFormat || currentScenario.name?.split(' → ')[1],
        sampleMessage: sampleMessage,
        similarTo: currentScenario.id?.includes('mt192') ? 'MT103' : 'MT202'
      };
